...
//设置字体
resetFont = function(titlefont,bodyfont,titleLineHeight,bodyLineHeight) {
    $('#news-article').css('font-size', bodyfont+"px");
    $('#news-title').css('font-size', titlefont+"px");
    $('p').css('line-height', bodyLineHeight+"px");
    $('#news-title').css('line-height', titleLineHeight+"px");
};
...
resetImage = function(idx, url, base64) {
    ...
    parent = elem.parent();
    elem.remove();
    return $('<img>').attr('src', base64).attr('imageIndex',idx).attr('id', 'image_ph_' + idx).attr('imageurl', url).addClass('news-image').appendTo(parent).on('click', showLargeImage);
};

//设置行情图片
resetStockImage = function(idx, url, base64 ,stockindex,hrefString) {
    var elem, parent;
    elem = $('#image_ph_' + idx);
    parent = elem.parent();
    elem.remove();
    return $('<img>').attr('src', base64).attr('stockIndex',stockindex).attr('hrefString',hrefString).attr('id', 'image_ph_' + idx).addClass('news-image').appendTo(parent).on('click', showStockDetail);
};

//放大图片操作
showLargeImage = function() {
    var $this, bounds,imageIndex, imageElem, imageId, imageURL;
    $this = $(this);
    imageElem = $this.get(0);
    imageElem.
    bounds = null;
    imageIndex = $this.attr('imageIndex')
    if (imageElem != null) {
        bounds = imageElem.getBoundingClientRect();
        bounds.width = imageElem.width;
        bounds.height = imageElem.height;
    ...
    return SNJavascriptBridge.callNativeBlock("jumptoimage", {
                                              "id": imageId,
                                              "galleryImageUrlString": imageUrl,
                                              "bounds": bounds,
...
function showStockDetail(){
    var $this, stockIndex,hrefString;
    $this = $(this);
    stockIndex = $this.attr('stockIndex');
    hrefString = $this.attr('hrefString');
    return SNJavascriptBridge.callNativeBlock("viewstock",{
...
(function (){
 ...
 var thistable = table;
 //找到基金表格的class
 var thisvalue =  thistable.getAttribute("class");
 if(thisvalue=="fund-tbl"){
 return;
 }
